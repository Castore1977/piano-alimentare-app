...
export default function App() {
  ...
  const generateMealPlan = async () => {
    ...
    let prompt = `Crea un piano alimentare di 7 giorni per una famiglia di ${validMembers.length} persone, includendo colazione, spuntino di metà mattina, pranzo, spuntino di metà pomeriggio e cena. I membri e i loro fabbisogni calorici giornalieri sono: ${JSON.stringify(membersData)}. I pasti devono essere gli stessi per tutti, ma le quantità devono essere personalizzate per ciascun membro. Per ogni pasto (inclusi gli spuntini), fornisci: nome del piatto e un array 'porzioni' con nome_membro, quantita_suggerita, calorie_porzione, e macronutrienti. La risposta DEVE essere un oggetto JSON valido.`;
  ...
  const fetchRecipe = async (meal) => {
    ...
    let prompt = `Fornisci una ricetta dettagliata per preparare "${meal.nome}" per ${familyMembers.length} persone. Elenca gli ingredienti con quantità totali e le istruzioni passo-passo. La risposta DEVE essere un oggetto JSON valido.`;
                        ...
                        <td key={mealType} className="p-4 align-top space-y-3">
                           <div className="flex justify-between items-start">
                               ...
                               <button onClick={() => openRegenModal(dayIndex, mealType)} disabled={isRegenerating !== null} className="text-gray-400 hover:text-emerald-400 transition-transform duration-200 hover:rotate-90 disabled:cursor-not-allowed disabled:opacity-50 flex-shrink-0">
                                   {isRegenerating?.dayIndex === dayIndex && isRegenerating?.mealType === mealType ? <Spinner size="h-5 w-5"/> : '🔄'}
                               </button>
                           </div>
